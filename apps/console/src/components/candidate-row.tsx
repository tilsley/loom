...
export function CandidateRow({ candidate, candidateRun, onQueue, onDequeue, isRunning }: CandidateRowProps) {
  const isBlocked = candidateRun?.status === "running" || candidateRun?.status === "completed";
  const hasRun = !!candidateRun?.runId;
  ...
  const row = (
    <div
      className={`grid grid-cols-[1fr_100px_1fr_80px] gap-x-2 px-3 py-2.5 ${
    ...
    >
      ...
      <div className="flex items-center gap-2 min-w-0 flex-wrap">
        ...
        <span className="text-[13px] font-mono text-zinc-300 truncate">{candidate.id}</span>
        {totalFiles > 0 && (
          <button
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); setFilesExpanded((v) => !v); }}
            className="inline-flex items-center gap-1 text-[10px] font-mono text-zinc-500 hover:text-zinc-300 transition-colors shrink-0"
          >
            ...
            {totalFiles} files
            <svg width="8" height="8" viewBox="0 0 12 12" fill="none" className={`transition-transform ${filesExpanded ? "rotate-180" : ""}`}>
      ...
      <div className="flex items-center gap-2 min-w-0">
        {hasRun ? (
          <>
            <span className="text-[11px] font-mono text-zinc-500 truncate">{candidateRun.runId}</span>
      ...
      <div className="flex justify-end" onClick={(e) => e.preventDefault()}>
        {candidateRun?.status === "queued" ? (
          <Button
            ...
            onClick={(e) => {
              e.stopPropagation();
              void onDequeue(candidateRun.runId);
            }}
            disabled={isRunning}
            className="text-[11px] py-1 px-2.5 text-zinc-400 hover:text-red-400 hover:border-red-500/30"
          >
            {isRunning ? "..." : "Cancel"}
          </Button>
        ) : (
            ...
            {fileGroups.map((group) => (
              <div key={group.name} className="min-w-0">
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-[10px] font-mono text-zinc-700">{group.repo}</span>
                </div>
                <div className="space-y-0.5">
                  {group.files.map((f) => (
                    <a
                      ...
                      className="flex items-center gap-1 text-[11px] font-mono text-zinc-500 hover:text-teal-400 transition-colors"
                    >
  ...
  if (hasRun) {
    return (
      <Link
        href={ROUTES.runDetail(candidateRun.runId)}
